...
if (!account || !userId || !manifestPath || !outputPath) {
  throw new Error('Usage: node scripts/collect-xhs-details-safe.mjs <account> <user-id> <manifest.json> <output.json>');
}

const batchSize = Math.max(1, Math.min(40, Number(process.env.XHS_DETAIL_BATCH_SIZE ?? 5)));
const batchDelayMs = Math.max(2_000, Number(process.env.XHS_DETAIL_BATCH_DELAY_MS ?? 30_000));
...
const complete = (row) => row?.detail_status === 'complete' && (row.is_video !== true || Boolean(row.media_url));
const pending = rows.filter((row) => !complete(existing.get(String(row.media_id ?? ''))));
process.stdout.write(`START ${account} candidates=${rows.length} complete=${rows.length - pending.length} pending=${pending.length}\n`);
...
for (let offset = 0; offset < pending.length; offset += batchSize) {
  ...
  if (run.status !== 0 || !Array.isArray(results) || !results.length) {
    throw new Error(`detail batch failed at offset ${offset}: ${(run.stderr || run.stdout || '').trim().slice(-500)}`);
  }
  ...
  process.stdout.write(`CHECKPOINT ${account} offset=${offset} returned=${results.length} total=${existing.size} failed=${failed.length}\n`);
  const failureLimit = Math.max(3, Math.ceil(results.length * 0.2));
  if (failed.length >= failureLimit) throw new Error(`detail stop: failed ${failed.length}/${results.length}`);
  if (offset + batch.length < pending.length) await new Promise((resolve) => setTimeout(resolve, batchDelayMs));
}

process.stdout.write(`DONE ${account} details=${existing.size}\n`);
